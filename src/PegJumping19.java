...
public class PegJumping19 {

	private static final int MAX_TIME = 14500;
	private static final boolean DEBUG = true;
	private static final byte NONE = 0;

	private long endTime = System.currentTimeMillis() + MAX_TIME;
	private int N, NN, N2, cp;
	...
	public String[] getMoves(int[] pegValue, String[] board) {
		...
		if (false) {
			...
			while (true) {
				...
				if (c == 'l')
					np -= 2;
				if (init.s[p] == NONE || init.s[np] != NONE || init.s[delPos(p, np)] == NONE)
					throw new RuntimeException();
				init = new State(init, new int[] { np, p }, init.s, 0);
				init.s[np] = init.s[p];
				init.s[p] = init.s[delPos(p, np)] = NONE;
			}
			endTime = System.currentTimeMillis() + MAX_TIME;
		}
		int maxw = 0;
	...
	int dist(int p1, int p2) {
		return Math.abs(getX(p1) - getX(p2)) + Math.abs(getY(p1) - getX(p2));
	}
	...
	private class State {
		...
		void output() {
			...
			for (int y = 0; y < N; ++y) {
				for (int x = 0; x < N; ++x) {
					System.out.print(String.format("%3d", s[getPos(y, x)]));
				}
				System.out.println(String.format("%4d", y));
			}
		}
		...
		private final int isFull(int p, byte[] s) {
			int res = 0, x = getX(p);
			if (x + 2 >= N || (s[p + 1] != NONE && s[p + 2] != NONE))
				++res;
			if (x - 2 < 0 || (s[p - 1] != NONE && s[p - 2] != NONE))
				++res;
			if (p + N2 >= NN || (s[p + N] != NONE && s[p + N2] != NONE))
				++res;
			if (p - N2 < 0 || (s[p - N] != NONE && s[p - N2] != NONE))
				++res;
			return res;
		}
